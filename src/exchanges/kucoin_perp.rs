...
impl ExchangeSource for KucoinPerpTicker {
    ...
    async fn run(&self, ctx: SourceContext) -> Result<()> {
        if self.symbols.is_empty() {
            anyhow::bail!("kucoin perp symbols empty");
        }
        ...
        let endpoint = bullet
            ...
            .instance_servers
            .first()
            .context("kucoin perp no instance server")?
            .endpoint
            .clone();
        ...
        for (i, s) in self.symbols.iter().enumerate() {
            let topic = format!("/contractMarket/ticker:{}", s);
            sink.send(Message::Text(
                json!({
                    "id": format!("sub-perp-{}", i),
                    "type":"subscribe",
                    "topic": topic,
        ...
        loop {
            tokio::select! {
                _ = ping_tick.tick() => {
                    if last_seen.elapsed() > Duration::from_secs(90) { anyhow::bail!("kucoin perp heartbeat timeout"); }
                ...
                msg = stream.next() => {
                    let msg = msg.context("kucoin perp stream ended")??;
                    match msg {
                        Message::Text(t) => {
                        ...
                        Message::Close(_) => anyhow::bail!("kucoin perp closed"),
                    }
                }
            }
        }

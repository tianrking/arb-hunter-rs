...
impl ExchangeSource for HtxPerpBbo {
    ...
    async fn run(&self, ctx: SourceContext) -> Result<()> {
        if self.symbols.is_empty() {
            anyhow::bail!("htx perp symbols empty");
        }

        let (ws, _) = connect_async("wss://api.hbdm.com/linear-swap-ws").await?;
        ...
        loop {
            tokio::select! {
                _ = ping_tick.tick() => {
                    if last_seen.elapsed() > Duration::from_secs(90) { anyhow::bail!("htx perp heartbeat timeout"); }
                    ctx.emit(DataEvent::Heartbeat { exchange: self.name(), ts_ms: now_ms() }).await?;
                }
                msg = stream.next() => {
                    let msg = msg.context("htx perp stream ended")??;
                    match msg {
                        Message::Binary(bin) => {
                        ...
                        Message::Close(_) => anyhow::bail!("htx perp closed"),
                    }
                }
            }
        }

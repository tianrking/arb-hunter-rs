...
impl ExchangeSource for BitgetPerpTicker {
    ...
    async fn run(&self, ctx: SourceContext) -> Result<()> {
        if self.symbols.is_empty() {
            anyhow::bail!("bitget perp symbols empty");
        }
        ...
        loop {
            tokio::select! {
                _ = ping_tick.tick() => {
                    if last_seen.elapsed() > Duration::from_secs(90) { anyhow::bail!("bitget perp heartbeat timeout"); }
                    sink.send(Message::Text("ping".into())).await?;
                    ctx.emit(DataEvent::Heartbeat { exchange: self.name(), ts_ms: now_ms() }).await?;
                }
                msg = stream.next() => {
                    let msg = msg.context("bitget perp stream ended")??;
                    match msg {
                        Message::Text(t) => {
                            if t == "pong" {
                        ...
                        Message::Close(_) => anyhow::bail!("bitget perp closed"),
                    }
                }
            }
        }

...
impl ExchangeSource for BitfinexPerpTicker {
    ...
    async fn run(&self, ctx: SourceContext) -> Result<()> {
        if self.symbols.is_empty() {
            anyhow::bail!("bitfinex perp symbols empty");
        }
        ...
        loop {
            tokio::select! {
                _ = ping_tick.tick() => {
                    if last_seen.elapsed() > Duration::from_secs(90) { anyhow::bail!("bitfinex perp heartbeat timeout"); }
                ...
                msg = stream.next() => {
                    let msg = msg.context("bitfinex perp stream ended")??;
                    match msg {
                        Message::Text(t) => {
                        ...
                        Message::Close(_) => anyhow::bail!("bitfinex perp closed"),
                    }
                }
            }
        }

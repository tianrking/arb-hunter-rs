...
impl ExchangeSource for OkxPerpTicker {
    ...
    async fn run(&self, ctx: SourceContext) -> Result<()> {
        if self.inst_ids.is_empty() {
            anyhow::bail!("okx perp symbols empty");
        }
        ...
        loop {
            tokio::select! {
                _ = ping_tick.tick() => {
                    if last_pong.elapsed() > Duration::from_secs(60) { anyhow::bail!("okx perp pong timeout"); }
                    sink.send(Message::Text("ping".into())).await?;
                    ctx.emit(DataEvent::Heartbeat { exchange: self.name(), ts_ms: now_ms() }).await?;
                }
                msg = stream.next() => {
                    let msg = msg.context("okx perp stream ended")??;
                    match msg {
                        Message::Text(t) => {
                        ...
                        Message::Close(_) => anyhow::bail!("okx perp closed"),
